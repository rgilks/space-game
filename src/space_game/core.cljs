...
(defn- receive-chat-msg [m]
  (let [id (aget m 0) cmd (keyword (aget m 1))]
    (js* "debugger;")
    (log (str id " : " cmd))
    )
  )
...
(defn on-key-down [state event]
  (send-chat-msg [(-> state :ship :id) (:key event)])
    (case (:key event)
      (:up) (update-in state [:ship :speed] faster)
      (:down) (update-in state [:ship :speed] slower)
      (:left) (assoc-in state [:ship :dir-change] -0.15)
      (:right) (assoc-in state [:ship :dir-change] 0.15)
      state)
